    ...
    >
      ...
      >
        <button
          onClick={onClose}
          className="
            cursor-pointer w-8 h-8 rounded 
            flex justify-center items-center bg-white/10 hover:bg-white/40 transition-colors
            absolute top-0 -right-11"
        >
...
UiModal.Body = function UiModalBody({ children, className }) {
  return <div className={clsx("px-6 flex-auto", className)}>{children}</div>;
};

UiModal.Footer = function UiModalFooter({ children, className }) {
  return (
    <div className={clsx("p-6 flex gap-4 justify-end", className)}>
      {children}
    </div>
  );
};
